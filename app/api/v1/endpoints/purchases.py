...
@router.post("")
def add_purchase(phone: str, amount: int, db: Session = Depends(get_db)):
    try:
        customer = get_customer_by_contact(db, phone)
        ...
        if hasattr(err, "status_code"):
            raise err
        logger.exception(f"Some Error Occurred in add_purchase(): {err}")
        raise response_parser.generate_response(
